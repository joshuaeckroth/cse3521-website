...
explanation. The heart is best explained in terms of its role in blood
circulation and relation to other organs. How to sort a list of
...
/implemented/ by /any/ next-lower level. For example, the Java
language (that is to say, the Java compiler and runtime environment)
may be implemented on any computational device. All computational
devices (pencil and paper plus a list of rules, or hydraulics and
tubes, or artificial neural networks, or cellular automata, etc.)
support the same computations. The symbol and algorithm level can be
implemented on any device from the processor and memory level.
