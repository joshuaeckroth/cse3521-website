...
problem-specific sense. A heuristic would be applied in step ~4.b.~,
...
search by modifying step ~4.b.ii.~ so that the entire "tocheck" list
is deleted and replaced with only those states accessible from the
current state. Also, step ~4.b.~ is modified to choose the best state
...
well as the heuristic value of that state. At step ~4.b.~, the best
