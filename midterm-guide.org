...
#+BEGIN_QUOTE
A robot wants to exit a maze. The maze is represented as a square
grid; open areas and walls are represented by grid squares.
#+END_QUOTE
...
~*~ I should have clarified: we have weighted graphs; breadth-first
search /is/ optimal for unweighted graphs.
