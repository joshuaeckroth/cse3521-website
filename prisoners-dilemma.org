...
Consider two players, X (she) and Y (he). Press and Dyson show that,
  ...
  - longer memory of the opponent's previous choices, beyond memory of
    just the single previous choice, serves no benefit
  ...
  - player X can set player Y's final score to any value in the range
    ...
    depending on what player Y chooses each round)

  - player X cannot set her own score in the same fashion
